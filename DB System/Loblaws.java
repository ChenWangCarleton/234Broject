	...
	public void productLevel(String source,boolean isLast,String cate) throws Exception {
		...
		if(e.size()==60) {
			if(isElementPresent(By.className("show-more-text"),web)) {
				...
				t.start();
				while(e.size()!=tot) {
					e=web.findElements(By.className("product-page-hotspot"));
					t.stop();
		...
		for(int x=0;x<e.size();x++) {
			...
			if(isLast&&x==e.size()-1) {
				fw.println("{\"category\":\""+cate+"\",\"name\":"+n+",\"description\":"+d+",\"price\":"+p+",\"brand\":"+b+",\"image\":"+i+",\"store\":\"Loblaws\"}");
			}
			else {
				fw.println("{\"category\":\""+cate+"\",\"name\":"+n+",\"description\":"+d+",\"price\":"+p+",\"brand\":"+b+",\"image\":"+i+",\"store\":\"Loblaws\"},");
			}
